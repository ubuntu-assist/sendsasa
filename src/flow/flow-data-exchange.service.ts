...
import { isPhoneNumber } from '../whatsapp/message-parser.service'
import { getAllBalances, isAccountActivated } from '../chains/xrpl.service'
...
import {
  ...
  CARD_FEE_PCT,
} from '../coinbase/coinbase-onramp.service'
...
export class FlowDataExchangeService {
  ...
  private static async handlePaydayTextInput(
    ...
    const { paydayService } = await import('../payday/payday.service')
  ...
  private static async handleDisputeEvidence(
    ...
    const phone = FlowDataExchangeService.extractWhatsappIdFromToken(flowData.flow_token)
    const { trustlockService } = await import('../trustlock/trustlock.service')
