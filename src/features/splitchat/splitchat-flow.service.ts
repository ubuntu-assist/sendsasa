...
@Injectable()
export class SplitChatFlowService {
  static async launchSplitChatCreateFlow(user: IUser): Promise<void> {
    ...
    await WhatsAppService.sendMessage({
      ...
      interactive: {
        type: 'flow',
        header: { type: 'text', text: '🎉 Group Collection' },
        body: { text: 'Collect money from multiple people for a shared event, gift, or project.' },
        ...
        action: {
          ...
          parameters: {
            ...
            flow_id: config.FLOW_ID_SPLITCHAT_CREATE,
            flow_cta: 'Create Pot',
            mode: 'published',
            flow_action: 'navigate',
            flow_action_payload: { screen: 'POT_DETAILS' },
          },
        },
      },
    })
