...
// ── Payout executor & cron poller ────────────────────────────────────────────

import { mobileMoneyService, PROVIDER_DISPLAY } from '@shared/mobile-money.service'
import { sendTextMessage } from '@messaging/whatsapp/whatsapp.service'
import { OnRampTransaction } from '@models/OnRampTransaction'

export async function executeOnRampPayout(
  onRampId: string,
  txHash?: string,
): Promise<void> {
  ...
  try {
    ...
    if (result.success) {
      ...
      logger.info(
        `[Coinbase] Payout completed: ${onRamp.xafAmount} XAF → ${onRamp.recipientPhone}`,
      )
...
export async function pollPendingOnRampTransactions(): Promise<void> {
  ...
  logger.info(
    `[Coinbase poller] Checking ${pending.length} pending transaction(s)`,
  )
  ...
  for (const onRamp of pending) {
    ...
    try {
      ...
      logger.info(
        `[Coinbase poller] Status check error for ${refId}: ${(err as Error).message}`,
      )
    }
  }
}
...
export class CoinbaseOnrampService {
  invalidateCredentialCache() {
    return invalidateCredentialCache()
  }
  createSessionToken(adminAddress: string, partnerUserRef: string) {
    return createSessionToken(adminAddress, partnerUserRef)
  }
  buildPaymentURL(sessionToken: string, presetFiatAmount: number) {
    return buildPaymentURL(sessionToken, presetFiatAmount)
  }
  calculateCardQuote(
    usdAmount: number,
    sendSasaRate: number,
    ...
    return calculateCardQuote(usdAmount, sendSasaRate, fixerRate)
  }
  createHeadlessOrder(params: Parameters<typeof createHeadlessOrder>[0]) {
    return createHeadlessOrder(params)
  }
  buildHeadlessPaymentURL(refId: string, baseUrl: string) {
    return buildHeadlessPaymentURL(refId, baseUrl)
  }
  verifyWebhookSignature(rawBody: Buffer, signatureHeader: string) {
    return verifyWebhookSignature(rawBody, signatureHeader)
  }
  saveCredentials(params: Parameters<typeof saveCredentials>[0]) {
    return saveCredentials(params)
  }
  saveWebhookSecret(secret: string) {
    return saveWebhookSecret(secret)
  }
  getTransactionStatus(partnerUserRef: string) {
    return getTransactionStatus(partnerUserRef)
  }
  createWebhookSubscription(notificationUrl: string) {
    return createWebhookSubscription(notificationUrl)
  }
  executeOnRampPayout(onRampId: string, txHash?: string) {
    return executeOnRampPayout(onRampId, txHash)
  }
  pollPendingOnRampTransactions() {
    return pollPendingOnRampTransactions()
  }
}
