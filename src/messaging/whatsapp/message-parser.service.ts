...
export function parseButtonInteraction(buttonId: string): ButtonInteraction {
  ...
  if (buttonId.startsWith('sell_asset:'))
    return { action: 'sell_asset', phone: buttonId.slice('sell_asset:'.length) }

  if (buttonId.startsWith('sell_confirm:'))
    return { action: 'sell_confirm', phone: buttonId.slice('sell_confirm:'.length) }
