...
import { FlowLauncherService } from '../flow/flow-launcher.service'
import {
  sendTextMessage,
...
import {
  ...
  createUSDCTrustLine,
  isAccountActivated,
} from '../chains/xrpl.service'
import { walletService } from '../chains/wallet.service'
import { evmService } from '../chains/evm.service'
import { getAllBalances as getSolanaBalances, sendSOL, sendUSDC as sendSolanaUSDC, sendUSDT as sendSolanaUSDT, sendEURC as sendSolanaEURC } from '../chains/solana.service'
import { normalizeToE164 } from '../services/phone-number.service'
import { mobileMoneyService, PROVIDER_DISPLAY, type MobileMoneyProvider } from '../services/mobile-money.service'
...
import { TrustLockFlowService } from '../trustlock/trustlock-flow.service'
import { NjangiFlowService } from '../njangi/njangi-flow.service'
import { SplitChatFlowService } from '../splitchat/splitchat-flow.service'
import { PayDayFlowService } from '../payday/payday-flow.service'
import { SafiPayFlowService } from '../safipay/safipay-flow.service'
...
import { KoboKallFlowService } from '../kobokall/kobokall-flow.service'
...
export async function handleMessage(
  ...
  try {
    ...
    if (['rates', 'rate', 'compare', 'exchange rate'].includes(normalizedText)) {
      const { formatRatesMessage } = await import('../services/rates.service')
...
export async function handleFlowResponse(
  ...
  try {
    ...
    if (hasPinConfirmedAction) {
      ...
      const { Deal } = await import('../trustlock/deal.schema')
...
async function handleBuyCrypto(
  ...
  const { createBuyLink } = await import('../onramper/onramper.service')
  ...
  try {
    ...
    const { sendCtaUrlButton } = await import('../whatsapp/whatsapp.service')
